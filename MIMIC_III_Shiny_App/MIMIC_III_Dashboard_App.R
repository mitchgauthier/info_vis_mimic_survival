...
setnames(mimic_data_full, old = c('gender','subject_id','ICU_LOS','risk_class','diagnosis','TEST_date_forecast_los_icu','icu_admit_age','TEST_date_forecast_los_hosp','HOSP_LOS','2012-01-01'), new = c('Gender','Name','Forecast ICU LOS','Risk_Class','Diagnosis','Forecast_Date_Leave_ICU','Age','Forecast_Date_Leave_Hospital','Forecast HOSP LOS','Current_Risk_Score'))
mimic_data_full$Forecast_Date_Leave_ICU <- as.Date(mimic_data_full$Forecast_Date_Leave_ICU, format='%Y-%m-%d')
mimic_data_full$Forecast_Date_Leave_Hospital <- as.Date(mimic_data_full$Forecast_Date_Leave_Hospital, format='%Y-%m-%d')
...
server <- function(input, output) {
  ...
  output$tbl = renderDT(
    select(mimic_data_full, c('Name', 'Gender','Age','Diagnosis','Forecast ICU LOS' ,'Forecast_Date_Leave_ICU','Current_Risk_Score','Risk_Class') )
  )
  
  output$supporting = renderDT(
    group_by(mimic_data_full, first_careunit) %>%
      summarise(
        days_until_hospital_exit = mean(days_until_hospital_exit))
  )
  ...
  output$scatterplot <- renderPlot({
    ...
    vis_metadata <- select(mimic_data_full, c('Name','Forecast_Date_Leave_ICU','Forecast_Date_Leave_Hospital','TEST_date_pass_away','TEST_date_leave_hospital','TEST_date_leave_icu')) %>%
  ...
  output$tbl_debugger = renderDT(
    select(mimic_data_full, c('Name', 'Gender','Age','Diagnosis' ,'Forecast_Date_Leave_ICU','Current_Risk_Score',
